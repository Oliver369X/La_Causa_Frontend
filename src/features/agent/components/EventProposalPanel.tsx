...
export type Proposal = {
  ...
  revision: number; payload: Draft; result?: { evento_id: string; titulo: string; tarea_id?: string; insignia_id?: string };
...
export function EventProposalPanel({ sessionId, orgId, traceId, refreshKey, disabled, onAdjust }: {
  ...
  useEffect(() => {
    let alive = true;
    agentClient.get<Proposal[]>("/proposals", { params: { session_id: sessionId, org_id: orgId, trace_id: traceId } })
      .then(({ data }) => { if (alive) { setProposals(data.filter(p => p.trace_id === traceId)); setLoadError(false); } })
  ...
  const decide = async (proposal: Proposal, decision: "prepare" | "save" | "confirm" | "cancel") => {
    ...
    try {
      ...
      setErrors(prev => ({ ...prev, [proposal.id]: typeof detail === "string" ? detail : "No se pudo completar la operación. Tus cambios siguen aquí; puedes reintentar." }));
      setReload(n => n + 1);
  ...
  const renderEditor = (p: Proposal) => {
    const props = { proposal: p, draft: drafts[p.id] || p.payload,
      ...
      decide: (d: "save" | "confirm" | "cancel") => void decide(p, d),
  ...
  const render = (p: Proposal) => {
    ...
    if (p.status === "review") return <div key={p.id} className="col-span-full space-y-2 border-l-2 border-[var(--border)] pl-4 text-sm">
      ...
      <button type="button" disabled={disabled || !!busy} onClick={() => void decide(p, "cancel")} className="text-xs text-[var(--text-muted)] underline">Descartar esta propuesta</button>
    ...
    if (p.status === "completed" || p.status === "cancelled" || p.status === "expired") return (
      <div key={p.id} className="rounded-xl border border-[var(--border)] px-4 py-3 text-sm">
        <strong>{d.titulo || "Propuesta"}</strong><p className="text-xs text-[var(--text-muted)]">{p.status === "completed" ? "Completado" : p.status === "cancelled" ? "Cancelada · no se creó nada" : "Propuesta vencida · no se creó nada"}</p>
    ...
    return <article key={p.id} className="min-w-0 rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] p-5 space-y-3">
      ...
      <div className="flex gap-3 text-sm">
        {p.status === "pending" && <button type="button" disabled={disabled || !!busy} onClick={() => { setExpanded(false); setEditing(p.id); }} className="rounded-lg bg-[var(--accent)] text-white px-3 py-2">Editar y crear</button>}
        <button type="button" disabled={disabled || !!busy} onClick={() => void decide(p, "cancel")} className="text-[var(--text-muted)]">Cancelar</button>
      </div>
      {errors[p.id] && <p role="alert" className="text-xs text-red-500">{errors[p.id]}</p>}
    </article>;
  };
  const reviewing = proposals.filter(p => p.status === "review");
  if (!proposals.length && !loadError) return null;
  return <div className="space-y-3 py-3" aria-label="Propuestas de este mensaje">
    ...
    {reviewing.length > 0 && <div className="rounded-xl bg-[var(--bg-subtle)] p-4 space-y-3">
      ...
      <div className="flex flex-wrap gap-3">
        <button type="button" disabled={disabled || !!busy} className="rounded-lg bg-[var(--accent)] text-white px-4 py-2 text-sm" onClick={async () => { for (const p of reviewing) await decide(p, "prepare"); }}>Continuar</button>
        <button type="button" disabled={disabled || !!busy} className="text-sm" onClick={onAdjust}>No, ajustar propuesta</button>
      </div>
    </div>}
    {proposals.length > 3 && <button type="button" className="text-sm text-[var(--accent)] underline" onClick={() => setExpanded(true)}>Ver todas ({proposals.length})</button>}
    <Modal open={expanded} onClose={() => { if (!busy) setExpanded(false); }} title="Propuestas del agente" size="2xl" scrollable>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">{expanded && proposals.map(render)}</div>
    </Modal>
    <Modal open={!!editing} onClose={() => { if (!busy) setEditing(null); }} title="Editar propuesta de tarea" size="2xl" scrollable>
